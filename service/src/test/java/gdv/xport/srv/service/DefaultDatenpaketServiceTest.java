...
public final class DefaultDatenpaketServiceTest {
    ...
    private static File TEST_FILE = toFile("../lib/src/test/resources/gdv/xport/test-export.txt");
    ...
    public void testValidateUri() {
        File musterdatei = toFile("../lib/src/test/resources/musterdatei_041222.txt");
        List<Model> violations = service.validate(musterdatei.toURI());
        assertEquals(0, violations.size());
    }

    private static File toFile(String filename) {
        return new File(FilenameUtils.normalize(new File(filename).getAbsolutePath()));
    }

    /**
     * Hier testen wir die eine provozierte Exception.
     */
    @Test
