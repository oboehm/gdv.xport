...
|baue alle Artefakte auf dem Release-Branch und rolle sie aus
(`mvn -Psign -DskipTests clean install source:jar javadoc:jar gpg:sign deploy -Dgpg.passphrase=xx`);
...
|D-Day
|JARs auf https://central.sonatype.com/p[central.sonatype.com] veröffentlichen
|gehe zu https://central.sonatype.com/publishing[Publishing] und betätige _Publishing Component_
...
|D-Day
|Label setzen
|`git tag gdv-xport-7.3.0` (lokal)
`git push git@github.com:oboehm/gdv.xport.git tag gdv-xport-7.3.0` (remote)
...
* Docker-Image taggen: `docker tag <Image-ID> oboehm/gdv.xport:7.3.0
* Docker-Hub anmelden: `docker login`
* Docker-Image pushen: `docker push oboehm/gdv.xport:7.3.0`
